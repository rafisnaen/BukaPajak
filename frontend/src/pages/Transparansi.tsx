...
import { getDetailedProvinceById, detailedProvinces, Project } from "@/data/detailed-provinces";
import { provinces } from "@/data/provinces";
import Footer from "@/components/Footer";
import { ProjectDetail } from "@/components/ui/ProjectDetailDialog";

// Mapper function to adapt dummy project data to the format expected by ProjectCard
const mapProjectToCardDetails = (project: Project): ProjectDetail => ({
  id: parseInt(project.id.split('-').pop() || '0', 10),
  name: project.name,
  description: project.description,
  budget: project.budget,
  image: project.stages[0]?.imageUrl || '/placeholder.jpg', // Use first stage image as placeholder
  status: project.status,
  category: project.category,
  startDate: project.startDate,
  averageRating: project.averageRating,
  ratings: project.ratings,
  comments: project.comments as any,
...
const Transparansi = () => {
  ...
  // If no provinceId is present, display all projects from all dummy provinces
  if (!provinceId) {
    const allProjects = detailedProvinces.flatMap(p =>
      [
        ...(p.projects || []).map(mapProjectToCardDetails),
        ...p.citiesData.flatMap(c => (c.projects || []).map(mapProjectToCardDetails)),
        ...p.citiesData.flatMap(c => c.districts.flatMap(d => (d.projects || []).map(mapProjectToCardDetails)))
      ]
    );
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="container mx-auto px-4 pt-24 pb-8">
          <div className="flex flex-col items-center justify-center text-center space-y-4 mb-12">
            <h1 className="text-4xl font-bold" style={{ color: '#3338A0' }}>
  ...
  // Get basic province data from dummy data
  const province = provinces.find(p => p.id === provinceId);

  // Get detailed province data from dummy data
  ...
  const mappedProvincialProjects = (detailedProvince?.projects || []).map(mapProjectToCardDetails);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 pt-24 pb-8">
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-green-600">{formatCurrency(province.remainingFunds)}</p>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <Badge className="ml-2 bg-green-500 text-white">{province.completedProjects}</Badge>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Berlangsung:</span>
                    <Badge className="ml-2 bg-yellow-500 text-white">{province.ongoingProjects}</Badge>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Provincial Projects */}
        {mappedProvincialProjects.length > 0 && (
          <Card className="mb-8">
            <CardHeader>
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {mappedProvincialProjects.map((project, index) => (
                  <ProjectCard key={`${project.id}-${index}`} project={project} />
                ))}
              </div>
            </CardContent>
